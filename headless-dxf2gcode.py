...
Features:
 ...
 - numerical tool radius compensation using shapely.buffer (keeps contours continuous)
 - groups contiguous geometry into contours and mills each contour in one continuous pass
...
try:
    ...
    print("Missing dependency: ezdxf. Install with: sudo pacman -S python-ezdxf or pip install ezdxf", file=sys.stderr)
    raise
...
try:
    ...
    print("Missing dependency: shapely. Install with: sudo pacman -S python-shapely or pip install shapely", file=sys.stderr)
    raise

try:
    import yaml
except Exception:
    print("Missing dependency: pyyaml. Install with: sudo pacman -S python-yaml or pip install pyyaml", file=sys.stderr)
    raise
...
def extract_geometries(dxf_path: str) -> List:
    """Return list of shapely geometries (LineString or Polygon) representing continuous entities."""
    ...
    for e in msp:
        ...
        if et == 'LINE':
            ...
            geoms.append(Polygon(LineString([(
                cx + math.cos(a) * r,
                cy + math.sin(a) * r
            ) for a in [i * 2 * math.pi / 64 for i in range(64)]]).coords))
        elif et == 'ARC':
            ...
            pts = [(cx + math.cos(sa + t * (ea - sa) / segments) * r,
...
def shift_geom(geom, dx: float, dy: float):
    return type(geom)([(x - dx, y - dy) for x, y in geom.coords]) if isinstance(geom, LineString) else geom.translate(-dx, -dy)

# Note: shapely's geometry objects don't have a generic 'translate' imported; use affinity.translate
...
def compensate_geometries_with_buffer(geoms: List, tool_radius: float, tool_side: str, buffer_resolution: int = 16, join_style: int = 1) -> List[Polygon]:
    """Return list of polygons representing compensated contours.
    join_style: 1=round, 2=mitre, 3=bevel (shapely conventions differ: actually 1 round, 2 mitre, 3 bevel)
    """
    if tool_radius == 0:
        return geoms
    ...
    for g in geoms:
        try:
            b = g.buffer(sign * tool_radius, resolution=buffer_resolution, join_style=join_style)
            if b.is_empty:
                continue
...
def merge_close_points(coords: List[Tuple[float, float]], tolerance: float = 0.01) -> List[Tuple[float, float]]:
...
def remove_short_segments(coords: List[Tuple[float, float]], min_length: float = 0.05) -> List[Tuple[float, float]]:
...
def douglas_peucker_simplify(coords: List[Tuple[float, float]], tolerance: float = 0.02) -> List[Tuple[float, float]]:
...
def calculate_segment_feedrate(length: float, base_feed: float, min_feed_ratio: float = 0.3) -> float:
...
def generate_gcode_from_polygons(polygons: List[Polygon], cfg: dict, enable_line_numbers: bool = False, line_inc: int = 10) -> List[str]:
    lines = []
    ln = 10
    ...
    # header
    emit(f"(Generated by headless-dxf2gcode with optimizations)")
    ...
    if optimization_enabled:
        emit(f"(Optimization enabled: merge_tol={merge_tolerance}, min_seg={min_segment_length}, dp_tol={douglas_peucker_tolerance})")
    ...
    for poly in polygons:
        ...
        for (x, y) in exterior[1:]:
            ...
            if optimization_enabled and adaptive_feedrate:
                ...
                current_feed = calculate_segment_feedrate(segment_length, base_feed, min_feed_ratio)
...
def plot_polygons(original_geoms, compensated_polys):
    ...
    for g in original_geoms:
        try:
            ...
            try:
                ...
                continue
        ax.plot(xs, ys, color='blue', linewidth=0.8, label='Original' if 'Original' not in ax.get_legend_handles_labels()[1] else '')
    # compensated
    for p in compensated_polys:
        xs, ys = zip(*list(p.exterior.coords))
        ax.plot(xs, ys, color='red', linestyle='--', linewidth=1.0, label='Compensated' if 'Compensated' not in ax.get_legend_handles_labels()[1] else '')
    # origin marker
...
def main():
    parser = argparse.ArgumentParser(description='DXF -> G-code with continuous contours and numerical tool offset')
    ...
    parser.add_argument('--origin-lower-left', action='store_true', help='Shift origin to lower-left corner (overrides config)')
    parser.add_argument('--plot', action='store_true', help='Show visualization of original and compensated shapes')
    parser.add_argument('--line-numbers', action='store_true', help='Enable N-line numbering (overrides config)')
    parser.add_argument('--no-optimize', action='store_true', help='Disable G-code optimizations (overrides config)')
    ...
    if tool_radius != 0:
        compensated_polys = compensate_geometries_with_buffer(geoms_shifted, tool_radius, tool_side, buffer_resolution, join_style)
    else:
        # if no compensation requested, convert geoms_shifted into polygons by buffering with tiny epsilon and taking their outlines
        compensated_polys = []
        for g in geoms_shifted:
            if isinstance(g, Polygon):
                compensated_polys.append(g)
            else:
                # create a very small buffer to get a polygon-like contour for milling along geometry
                p = g.buffer(0.0001)
                if not p.is_empty:
                    if isinstance(p, Polygon):
    ...
    if cfg.get('preamble'):
        if os.path.exists(cfg['preamble']):
            ...
            header_lines.append(f"(WARNING: preamble file not found: {cfg['preamble']})")
    ...
    if cfg.get('postamble'):
        if os.path.exists(cfg['postamble']):
            ...
            footer_lines.append(f"(WARNING: postamble file not found: {cfg['postamble']})")

    body_lines = generate_gcode_from_polygons(compensated_polys, cfg, enable_line_numbers=enable_ln, line_inc=line_inc)
    ...
    if header_lines:
        if enable_ln:
            ...
            # ensure body starts after header numbers - but generate_gcode_from_polygons already started at N10; keep it simple and just concatenate
